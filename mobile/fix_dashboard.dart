...
void main() {
  ...
  if (content.contains('final String _selectedVoucherType = "All Types";')) {
  ...
  } else if (!content.contains('_selectedAccountType')) {
    content = content.replaceFirst(
        'String _selectedVoucherType = "All Entries";',
        'String _selectedVoucherType = "All Entries";\n  String _selectedAccountType = "All Accounts";');
  }
  ...
  if (startIdx != -1) {
    content = content.substring(0, startIdx) + replacementText;
    file.writeAsStringSync(content);
    print("Dashboard updated successfully.");
  } else {
    print("Could not find VOUCHERS PAGE marker.");
  }
}

const replacementText = r'''// --- VOUCHERS PAGE ---
        ...
        Column(
          ...
          children: [
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: ["All Entries", "Sales", "Purchase", "Payment", "Receipt", "Journal"].map((f) {
            ...
            Expanded(
              child: filteredVouchers.isEmpty ? Center(child: Text("No entries found.", style: GoogleFonts.plusJakartaSans(color: isDark ? Colors.grey : Colors.grey[700]))) : ListView.separated(
                ...
                separatorBuilder: (c, i) => Divider(color: Colors.grey.withOpacity(0.2), height: 1),
  ...
  Widget _buildAccountsContent() {
    ...
    var filtered = _accounts.where((a) {
      if (_selectedAccountType != "All Accounts" && _selectedAccountType != "All" && _selectedAccountType != "") {
        ...
        if (_selectedAccountType.toLowerCase() == 'income' && t != 'income') return false;
        if (_selectedAccountType.toLowerCase() == 'expense' && t != 'expense') return false;
      }
    ...
    return Column(
      children: [
        ...
        SingleChildScrollView(
          ...
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: Row(
            children: ["All Accounts", "Assets", "Liabilities", "Equity", "Income", "Expense"].map((f) {
              bool sel = _selectedAccountType == f || (_selectedAccountType == "" && f == "All Accounts");
              return GestureDetector(
                onTap: () => setState(() => _selectedAccountType = f == "All Accounts" ? "All" : f),
                child: Container(
                  margin: const EdgeInsets.only(right: 8),
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              Text("ACCOUNT & CATEGORY", style: GoogleFonts.plusJakartaSans(color: Colors.grey, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
              Text("BALANCE", style: GoogleFonts.plusJakartaSans(color: Colors.grey, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
            ]
          )
        ),
        ...
        Expanded(
          child: filtered.isEmpty ? Center(child: Text("No accounts found.", style: GoogleFonts.plusJakartaSans(color: isDark ? Colors.grey : Colors.grey[700]))) : ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              else if (a['type'] == 'Income') dotColor = Colors.purple;
              return Padding(
                  ...
                  children: [
                    ...
                    Expanded(child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 2),
                        Text("${a['type']} • ${a['sub_type'] ?? a['type']}", style: GoogleFonts.plusJakartaSans(color: Colors.blueGrey, fontSize: 13)),
                      ]
                    )),
                    Text("\$${b.toStringAsFixed(2)}", style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.bold, fontSize: 16, color: a['type'] == 'Liability' || a['type'] == 'Expense' ? (isDark ? Colors.redAccent : const Color(0xFFEF4444)) : (isDark ? Colors.white : Colors.black87))),
                  ]
                )
              );
            },
